...
### Added
- **Server status indicator** — a colored dot on each row shows real health:
  green when the server is responding to HTTP, amber while it's listening or
  starting, red if a start failed, grey when stopped. Reachability is checked
  with a lightweight HEAD probe after each scan, so it reflects "actually
  serving" — not just "port is open".
...
[1.5.0]: https://github.com/zehash-developer/PortHole/compare/v1.4.0...v1.5.0
